...
```{r}
getwd()
setwd("~/Desktop/working/directory/Group5")
...
#21 unique values; should be 3 - need to remove 18
...
#### **Checking for entirely duplicated entries**
...
- many experiment repeats for the same mouse model & phenotype (diff analysis_id & pvalue)
- also present are duplicated entries (diff analysis_id, same pvalue)
...
#### checking for multiple entries for the same mouse model & phenotype - do they have pvalue duplicates ?
...
#identifies all entries where duplications are present, as well as alternative experimental pvalues 

```

#### **Checking for duplicated p-values of entries with the same column values excluding analysis_id**

```{r}

### Method One to try and identify the duplicated p-values
...
### Method Two - trying via grouping columns 2-6 
...
### Method Three - Alternative method from other group members 
